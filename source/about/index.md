title: About Me
date: 2016-02-13 14:58:08
comments: false
---
<span class="img-left">
![Hi! it's me!](/images/profile-picture.jpg)
</span>

## Basic Information
English Name: Tzu-Chao, Wang
Chinese name: 汪子超
Birthday: 2<sup>10</sup> (October 24)
Constellation: Scorpio

## Contact Information
- E-mail: john29917958ATgmailDOTcom
- [ArtStation](https://www.artstation.com/tzuchaowang)
- [Pixiv](https://www.pixiv.net/member_illust.php?id=24328892&type=illust)
- [LinkedIn](https://www.linkedin.com/in/john29917958)
- [Facebook](https://www.facebook.com/tzuchaowang)
- [Instagram](https://www.instagram.com/john29917958/)
- [Twitter](https://twitter.com/john29917958)
- [Tumblr](https://www.tumblr.com/blog/john29917958)

## Work Experience

### Taiwan Semiconductor Manufacturing Company Limited
Daya, Taichung
2017/08 ~ now
Computer Integrated Manufacturing Engineer

---

### NCU Object-Oriented Lab
Chungli, Taoyuan
2015/08 ~ 2017/07

#### KORAT
A powerful regression test automation system with capture / replay ability.

Positions: Chief programmer / architecture
Challenges and Contributions:
- Bring the KORAT prototype into a production level system
- Accomplish production deployment and installer for KORAT
- Ensure the stability of KORAT in the application of [ADLINK inc.](http://www.adlinktech.com/big5/index.php)
- Dominant and solve the multi-threading, multi-process issues of KORAT
- Dominant and solve cross language integration (C# and C++) issues of KORAT

#### POC Web Prototype
A real-time web application for ADLINK to monitor the state of machines in SMT production line.

Positions: PM / Full-stack web engineer
Challenges and Contributions:
- Dominant entire software process and implementation of POC app
- Deliver the POC prototype to ADLINK within only 1 month

#### Network Administrator
Challenges and Contributions:
- Maintain the GitLab, Backup servers of OO Lab
- Add HTTPs and auto-renewing of HTTPs certificate of GitLab server
- Write comprehensive maintenance document for GitLab and Backup server
- Maintain the Nine-Grids compiler course system written in Ruby-on-Rails
- Write all-in-one Nine-Grids installation shell script (complete installation with `./deploy.sh`)

---

### NCU Computer Center Service Desk
Chungli, Taoyuan
2014/08 ~ 2017/08

#### Service Desk Customer Service Web System
A platform for students in NCU to propose questions and find solutions for their daily life in NCU: [https://sd.cc.ncu.edu.tw/](https://sd.cc.ncu.edu.tw/)

Positions: Full-stack web engineer / Consultant
Challenges and Contributions:
- Bring the idea of Service Desk into a real, production web system
- Own and develop more than half of the features of whole system
- Dominant and push the deployment and maintenance of the production web server
- Design and execute the freshman cultivation plan with teammates
- Write comprehensive installation and maintenance guide for newcomers.

---

### NCU CHST College
Chungli, Taoyuan
2014/08 ~ 2015/07

### Network Administrator
Contributions and Challenges:
- Migrate the production web server of CHST into Docker container
- Manage the maintenance and security of the web server
- Manage all graphic design tasks of CHST

## Professional Skills
**Programming Languages**
C#, JavaScript, CSS3, HTML5, PHP, Java, Node.js

**Frameworks / Libraries**
Bootstrap, Material Design (MaterializeCSS)
jQuery, ReactJS (a little)
Laravel, MeteorJS, SailsJS

**Project Development Tools**
UML, Git, Slack, Git-flow, MySQL, MariaDB, Visual Studio, Eclipse, Sublime Text

**Media Processing Tools**
Adobe PhotoShop, Illustrator, Premiere Pro, Audition
Here is my [Design collections](/categories/Graphic-design/)

## Languages
Chinese: Natural language
English: TOEIC 690, better at reading and writing

## Interests
1. Dancing and listen to music
2. Programming (especially web)
3. Drawing
4. Watch movies or Animes

## Education
- [國立中央大學](http://www.ncu.edu.tw/) [資訊工程學系](http://www.csie.ncu.edu.tw/) [Object-Oriented Lab](http://oolab.csie.ncu.edu.tw/wiki/index.php/OO_Lab) (currently studying in)
- [國立中央大學](http://www.ncu.edu.tw/) [資訊工程學系](http://www.csie.ncu.edu.tw/)
- [國立林口高級中學](http://www.lksh.ntpc.edu.tw/)
- [新泰國民中學](http://www.htjh.ntpc.edu.tw/)
- [新泰國民小學](http://www.stps.ntpc.edu.tw/)