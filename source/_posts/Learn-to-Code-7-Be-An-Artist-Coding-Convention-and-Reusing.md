...
## 技巧五: Reusing
你知道嗎? 頂尖的軟體工程師都會在適當的時候把「重複程式碼」包裝起來，讓他像工具一樣可以在很多地方被使用。趕快把功能寫好可以很快有進度，但是人們不一定知道，當程式慢慢成長，重複的程式碼也常跟著出現。當類似的程式碼出現了兩三次，會很有機會出現第四、第五、甚至是第六次! 把重複出現的程式碼用有意義的名稱包裝起來重複使用，反而可以提高未來的工作效率，也可以讓程式碼更精簡。甚至如果未來有需要優化這段邏輯，只要修改包裝好的 function 就好，而不用到專案裡把類似的功能全部找出來，再一個一個去修改。讓我給你一個例子:
