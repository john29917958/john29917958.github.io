...
程式碼撰寫風格是用一種統一約定好的格式來命名你的變數、功能、屬性、類別，或是用約定好的方式來排版程式碼等等。遵循好的程式碼撰寫風格可以避免寫出隱含 bug 的程式碼、讓程式碼更容易被閱讀、比較有段落，也比較容易可以讓我們一眼看出變數的類型跟特性。專案成員有異動的時候，也會更容易被 交接。我們先看來一個範例! 如果我們宣告兩個變數，而且都用它們記錄學生的身高:
