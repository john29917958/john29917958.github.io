...
這裡我想要你先跟我一起想想，寫信的時候如果要用英文寫「學生的身高」，我們會寫「student height」，用空白來把兩個單字隔開，但空白卻不準出現在變數名稱裡面。為了讓程式碼更貼近文章，我們可以用底線「`_`」，把第一個單字跟第二個單字隔開，讓讀者在看「`student_height`」的時候產生有兩個單字的視覺效果。這比起把兩個單字黏在一起的 `studentheight` 好讀多了! 這種用底線「`_`」把單字隔開，而且規定變數名稱裡的每個字元都要保持小寫的規則有個統一的名稱叫「Snake Case - 蛇形命名法」，是一種程式碼的「命名規則 (naming convention)」。C、C++ 跟 Python 的變數名稱就都應該遵循 Snake Case 命名風格。
