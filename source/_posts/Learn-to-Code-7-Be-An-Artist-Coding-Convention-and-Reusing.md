...
非常好! 三個步驟的細節已經全部被從主迴圈移到三個 functions 裡面。第一個 function 只執行跟輸入輸出有關的指令、第二個 function 只執行跟場景有關的指令、第三個 function 只執行跟玩家有關的指令。每個 function 各司其職，完全不做跟目的沒關係的事。把三個 functions 都搬出去以後，主邏輯也變得非常簡單易懂，而且可以一眼就看出這份遊戲主要有那些步驟，也可以大略知道每個步驟主要在做什麼事。最後，我們要談的是今天最進階的主題: Reusing。
