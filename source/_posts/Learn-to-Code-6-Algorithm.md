...
我們今天要談的是怎麼寫出「執行時間」或「使用的資源」符合期待的程式。想像一下! 如果上傳一張圖到 Google 並按下搜尋按鈕之後，苦苦等了五分鐘才得到結果。這樣的執行時間顯然很難讓人接受。人們可能完全沒有想過為什麼要了解演算法? 為什麼要在寫 code 之前先設計邏輯、設計步驟? 為什麼要先計算程式最糟的執行時間跟耗費的資源? 如果你正頭痛該怎麼設計解決方案、如果你想從會 coding 的程度進一步突破，我想要你點開這篇文章，讓我跟你分享設計演算法的秘密!
