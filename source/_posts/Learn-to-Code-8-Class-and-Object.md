...
## 主題一: Class 與物件
現在你知道了，當系統裡的物件不斷被擴充，類似的程式碼有機會跟著出現。如果沒有管理好 code，程式碼也會變得非常難維護。不要再這麼寫了! 你需要 class 與物件的觀念跟技巧來解決這些問題! 如果程式碼可以像 3D 列印的模型一樣，做好 model、設定好參數之後，交給 3D 列印機印出實物來。每個實物可以有自己的風格例如配色，而且要印多少就印多少，這不是非常好嗎?
