...
上面這段 code 是模擬在特定條件下去發動人物的技能。我想要你看看 code 裡的 `if` statement。在我認為，這段判斷式有點冗長，需要花較大的力氣去理解它。其實我們可以用一個技巧把程式碼包裝起來，讓程式碼變得更好讀:
...
你看，`if` statement 裡複雜的條件已經用 `should_unleash_skill` 這個 `bool` 變數包裝起來。`if` 裡的條件其實可以
變得清楚又好懂，只要看變數名稱就已經大略可以了解判斷式的目的。最後，我想要你看這段優化過的程式碼:
