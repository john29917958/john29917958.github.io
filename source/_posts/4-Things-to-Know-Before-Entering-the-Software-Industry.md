...
程式，其實就是些文字檔。就像英文與中文，程式也有自己的語法、語意跟邏輯。程式被撰寫完之後，會被「編譯」成可以「點兩下」，或用指令跑起來的執行檔。有些語言撰寫完後，則會被儲存成「腳本」文件，而不需要經過「編譯」的動作。接著，會由電腦、手機或是瀏覽器執行之。到這個階段，就是我們平常看到的 App 或是網頁了：
1. 撰寫程式
2. 編譯成執行檔，或儲存成「腳本」文件
3. 執行
...
{% asset_img maintenance-effort.png "維護難度指數上升示意圖'維護難度指數上升示意圖'" %}
