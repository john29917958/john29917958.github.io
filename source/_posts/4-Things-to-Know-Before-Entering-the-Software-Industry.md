...
程式設計是一項高收入技能，程式設計有著無窮的潛力。決定挑戰「程式技能」的你，在未來發展方面，可以先給自己掌聲鼓勵 XD。至於「考高分」、「標準答案」、「填鴨教育」在程式的世界為何行不通？It's another story，在此僅能簡單述說，程式看的是根據千變萬化的需求做出抽象設計，並加以艱深的程式碼撰寫出來。在不同領域下的類似情境，設計手法與寫法都不盡相同。
